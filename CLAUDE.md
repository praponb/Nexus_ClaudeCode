...
- The tunnel LaunchDaemon (`/Library/LaunchDaemons/com.cloudflare.cloudflared.plist`)
  is `launchctl disable`d in the **system** domain — confirmed by reading the
  override back on 2026-08-31, so it does **not** return at boot and the Mac is
  safe to restart. Re-enabling it is the rollback step, and must never run while
  the server's tunnel is up — one tunnel, one host, or Cloudflare load-balances
  across two diverging databases.
- That override is the *only* thing holding it: the plist is still installed and
  still `RunAtLoad`, and the rollback's `launchctl enable` re-arms it. So after
  any rollback, re-run the check in `SESSION-2026-08-29-ISSUES.md` §8 before the
  next reboot — `bootout` alone lasts only for the current boot.
